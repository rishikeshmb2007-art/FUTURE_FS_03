...
const FoodCard = ({ image, name, description, price, isVeg, theme }) => {
  ...
  // Fallback image if the original link breaks
  const handleImageError = (e) => {
    e.target.onerror = null; // Prevents infinite loop if fallback also breaks
    ...
    >
      ...
      <div className="p-5">
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          <button className="bg-white/10 hover:bg-white/20 text-white p-2 rounded-full transition-colors border border-white/10">
            ➕
          </button>
        </div>
      </div>
